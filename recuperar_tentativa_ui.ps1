...
if ($LASTEXITCODE -ne 0) {
    throw "Falha ao restaurar os templates pelo Git."
}
...
$messagesTracked = git ls-files --error-unmatch $messagesPath 2>$null
...
if (Test-Path $badScript) {
    Move-Item $badScript (Join-Path $backup "aplicar_ui_foundation_01_com_erro.ps1") -Force
}
...
Set-Content -Path ".\diagnostico_recuperacao_ui.txt" -Value $report -Encoding UTF8
