...
def registrar_saida_estoque(
    *,
    ...
    try:
        return _registrar_saida_estoque_atomic(
            matriz=matriz,
            loja=loja,
            produto=produto,
            tipo=tipo,
...
@transaction.atomic
def _registrar_saida_estoque_atomic(
    *,
    matriz,
    loja,
    produto,
...
def _validar_tipo_saida(*, tipo):
    natureza = get_natureza_tipo_movimentacao(tipo)

    if natureza != NaturezaMovimentacao.SAIDA:
        raise ValidationError({
            'tipo': (
                'O tipo informado não representa uma saída de estoque.'
            )
        })
...
def _obter_saldo_bloqueado(
    *,
    ...
    if saldo is None:
        raise ValidationError({
            'saldo': (
                'Não existe saldo de estoque para o produto '
                'na loja informada.'
            )
        })
