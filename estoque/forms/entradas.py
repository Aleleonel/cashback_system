"""Formulário de entrada manual de estoque."""
...
def obter_tipos_entrada():
    return [
        (valor, rotulo)
        for valor, rotulo in TipoMovimentacao.choices
        if get_natureza_tipo_movimentacao(valor) == NaturezaMovimentacao.ENTRADA
    ]
...
class EntradaEstoqueForm(forms.Form):
    ...
    documento_referencia = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'placeholder': 'Nota fiscal, pedido ou recebimento',
        }),
    )
    ...
    def __init__(self, *args, matriz, loja_inicial=None, **kwargs):
        ...
        self.fields['tipo'].choices = obter_tipos_entrada()
    ...
    def clean_tipo(self):
        ...
        if get_natureza_tipo_movimentacao(tipo) != NaturezaMovimentacao.ENTRADA:
            raise forms.ValidationError('Selecione um tipo válido de entrada.')
        return tipo
