...
def menu_permissoes(request):
    ...
    return {
        ...
        'pode_nova_compra': usuario_tem_permissao(
            usuario,
            PERMISSAO_CASHBACK_NOVA_COMPRA
        ),
        'pode_ver_clientes': usuario_tem_permissao(
            usuario,
            PERMISSAO_CLIENTES_VISUALIZAR
        ),
        'pode_importar_clientes': usuario_tem_permissao(
            usuario,
            PERMISSAO_CLIENTES_IMPORTAR
        ),
        'pode_ver_produtos': usuario_tem_permissao(
            usuario,
            PERMISSAO_PRODUTOS_VISUALIZAR
        ),
        'pode_importar_produtos': usuario_tem_permissao(
            usuario,
            PERMISSAO_PRODUTOS_IMPORTAR
        ),
        'pode_ver_campanhas': usuario_tem_permissao(
            usuario,
            PERMISSAO_CAMPANHAS_DISPARAR
        ),
        'pode_configurar_campanhas': usuario_tem_permissao(
            usuario,
            PERMISSAO_CAMPANHAS_CONFIGURAR
        ),
        'pode_templates_campanhas': usuario_tem_permissao(
            usuario,
            PERMISSAO_CAMPANHAS_TEMPLATES
        ),
        'pode_empresa_lojas': usuario_tem_permissao(
            usuario,
            PERMISSAO_EMPRESA_LOJAS_GERENCIAR
        ),
        'pode_empresa_usuarios': usuario_tem_permissao(
            usuario,
            PERMISSAO_EMPRESA_USUARIOS_GERENCIAR
        ),
        'pode_empresa_cashback': usuario_tem_permissao(
            usuario,
            PERMISSAO_EMPRESA_CONFIGURAR_CASHBACK
        ),
        'pode_gerenciar_vouchers': usuario_tem_permissao(
            usuario,
            PERMISSAO_VOUCHERS_GERENCIAR
        ),
    }
